...
# Lab 2 SI 618: Fetching and parsing structured documents
#
...
def get_pigs():
  ...
  soup = BeautifulSoup(html_doc)
  tmp = []
  for i in soup.body.ol.find_all('h2'):
    tmp.append(i.string)
...
def get_story_headings():
  ...
  soup = BeautifulSoup(html_doc)
  tmp = []
  for i in soup.body.find_all('h2'):
    if 'Story' in i.text: tmp.append(i.string)
  return json.dumps(tmp)
...
def get_houses():
  ...
  for i in soup.find_all('tr')[1:]:
    pig = i.contents[1].text
    house = i.contents[3].text
    tmp.append((pig,house))
...
def treasure_hunting():
  # Your code here: make sure to modify the return statement to pass back
  # the correct value.
  html_doc = urllib2.urlopen('http://www.example.com').read()
  soup = BeautifulSoup(html_doc)
  url = soup.find_all('a')[0]['href']
  soup = BeautifulSoup(urllib2.urlopen(url).read())
  return soup.find_all('tr')[3].td.string
